      ...
      <div style={{ width:"100%", maxWidth:360 }}>
        ...
        {!showPass ? (
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            {USUARIOS.map(u => (
              <button key={u.key} onClick={() => select(u)}
                style={{ background:u.color, border:"none", borderRadius:16, padding:"24px 12px", cursor:"pointer", color:"#fff" }}>
          ...
          <div style={{ background:"#fff", borderRadius:16, padding:28 }}>
            ...
            <input type="password" value={pass} onChange={e => setPass(e.target.value)}
              ...
              style={{ width:"100%", padding:"9px 12px", borderRadius:8, border:"1px solid #ddd", fontSize:14, marginBottom:10, boxSizing:"border-box" }} />
...
function VendedorApp({ user, onLogout }) {
  ...
  const refresh = useCallback(async () => {
    try {
      const [o, s, a] = await Promise.all([
        apiGet({ action: "getPedidos" }),
        apiGet({ action: "getStock" }),
        apiGet({ action: "getArticulos" }),
      ]);
  ...
  const enviar = async () => {
    ...
    setSyncing(true);
    const o = { id: Date.now(), vendedor: user.name, cliente: cliente.trim(), tipo, promo, urgente, items, total, fecha: new Date().toLocaleString("es-AR"), estado: "pendiente", estadoReparto: "", fechaReparto: "" };
    await apiPost("addPedido", o);
    const latest = await apiGet({ action: "getPedidos" });
  ...
  return (
    <div style={{ maxWidth:500, margin:"0 auto", fontFamily:"sans-serif", background:"#f4f6fb", minHeight:"100vh" }}>
      <Header user={user} syncing={syncing} onLogout={onLogout} />
            ...
            <div style={cd}>
              ...
              <div style={{ maxHeight:220, overflowY:"auto" }}>
                {filtered.slice(0, 80).map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => addItem(p)} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 10px", borderRadius:8, marginBottom:4, background:"#f8fafc", cursor:"pointer", border:"1px solid #e8edf5" }}>
                      <div style={{ flex:1, minWidth:0 }}>
                        <b style={{ fontSize:12, display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.nombre}</b>
              ...
              <div style={cd}>
                ...
                {items.map(i => (
                  <div key={i.id} style={{ display:"flex", alignItems:"center", gap:8, marginTop:8, padding:"8px", background:"#f0f4ff", borderRadius:8 }}>
                    <div style={{ flex:1, minWidth:0 }}>
                      <b style={{ fontSize:12, display:"block", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{i.nombre}</b>
                    ...
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      ...
                      <input type="number" value={i.qty} onChange={e => updQty(i.id, e.target.value)}
                        style={{ width:48, textAlign:"center", fontWeight:700, fontSize:15, border:"1px solid #ddd", borderRadius:8, padding:"4px 0" }} />
                      <button onClick={() => updQty(i.id, i.qty + 1)} style={qB}>+</button>
                    </div>
            ...
            {myOrders.map(o => (
              <div key={o.id} style={{ ...cd, borderLeft:`4px solid ${o.urgente ? "#e53935" : "#43a047"}` }}>
                <div style={{ display:"flex", justifyContent:"space-between" }}>
                  <div>
                    <b>{o.cliente}</b>
                    <span style={{ fontSize:11, marginLeft:6, background:o.tipo === "pedido" ? "#e3f0ff" : "#fff3e0", color:o.tipo === "pedido" ? "#1e3a5f" : "#e65100", borderRadius:6, padding:"2px 6px" }}>{o.tipo}</span>
                    <div style={{ fontSize:12, color:"#888", marginTop:2 }}>{o.fecha}</div>
                  </div>
                ...
                {o.fechaReparto && <div style={{ fontSize:12, color:"#e65100", marginTop:2 }}>📅 Reparto: {o.fechaReparto}</div>}
                <div style={{ fontWeight:700, color, marginTop:6 }}>Total: ${o.total.toLocaleString("es-AR")}</div>
                {o.tipo === "presupuesto" && (
                  <a href={`https://wa.me/?text=${encodeURIComponent(txtWA(o))}`} target="_blank" rel="noreferrer"
                    style={{ display:"block", textAlign:"center", background:"#25D366", color:"#fff", borderRadius:10, padding:"10px 0", textDecoration:"none", fontWeight:700, marginTop:8 }}>
                    📱 Mandar por WhatsApp
                  </a>
                )}
              </div>
        ...
        {tab === "precios" && (
          <div>
            ...
            {filtered.slice(0, 100).map(p => (
              <div key={p.id} style={{ ...cd, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 12px" }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <b style={{ fontSize:12 }}>{p.nombre}</b>
                  <div style={{ fontSize:10, color:"#888" }}>{p.codigo} · {p.proveedor}</div>
                </div>
                <b style={{ color, fontSize:13, marginLeft:8 }}>${p.precio.toLocaleString("es-AR")}</b>
              </div>
            ))}
          </div>
        )}
      ...
      {showRes && (
        <Modal onClose={() => setShowRes(null)}>
          ...
          <button onClick={() => { setShowRes(null); setTab("nuevo"); }} style={{ ...bP(color), marginBottom:8 }}>➕ Cargar otro pedido</button>
          <button onClick={() => { setShowRes(null); setTab("mis-pedidos"); }} style={bP("#888")}>📋 Ver mis pedidos</button>
        </Modal>
      )}
    </div>
  );
...
function RepartidorApp({ user, onLogout }) {
  ...
  const refresh = useCallback(async () => {
    try { const o = await apiGet({ action: "getPedidos" }); if (Array.isArray(o)) setOrders(o); }
    catch (e) { console.error(e); }
    setLoading(false);
  }, []);
  ...
  const updReparto = async (id, estadoReparto, fechaReparto = "") => {
    setOrders(prev => prev.map(o => o.id === id ? { ...o, estadoReparto, fechaReparto } : o));
  ...
  return (
    <div style={{ maxWidth:500, margin:"0 auto", fontFamily:"sans-serif", background:"#f4f6fb", minHeight:"100vh" }}>
      ...
      <div style={{ display:"flex", background:"#fff", borderBottom:"2px solid #e8edf5", overflowX:"auto" }}>
        {[
          { k:"pendiente", label:`📋 Sin asignar (${counts.pendiente})` },
          { k:"en_reparto", label:`🚚 En reparto (${counts.en_reparto})` },
          { k:"programado", label:`📅 Programado (${counts.programado})` },
          { k:"entregado", label:`✅ Entregado (${counts.entregado})` },
        ].map(t => (
          <button key={t.k} onClick={() => setTab(t.k)} style={{ flex:"0 0 auto", padding:"11px 14px", border:"none", background:"transparent", borderBottom:tab === t.k ? `3px solid ${color}` : "3px solid transparent", color:tab === t.k ? color : "#888", fontWeight:tab === t.k ? 700 : 400, cursor:"pointer", fontSize:12, whiteSpace:"nowrap" }}>
            {t.label}
          </button>
        ))}
      </div>
        ...
        {filtered.map(o => (
          <div key={o.id} style={{ ...cd, borderLeft:`4px solid ${o.estadoReparto === "entregado" ? "#43a047" : o.estadoReparto === "en_reparto" ? "#e65100" : o.estadoReparto === "programado" ? "#1565c0" : "#999"}` }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
              <div>
                <b style={{ fontSize:15 }}>{o.cliente}</b>
                <div style={{ fontSize:12, color:"#888" }}>{o.vendedor} · {o.fecha}</div>
              </div>
              <RepartoBadge estadoReparto={o.estadoReparto} fechaReparto={o.fechaReparto} />
            </div>
            ...
            <div style={{ display:"flex", gap:8, marginTop:10, flexWrap:"wrap" }}>
              ...
              <button onClick={() => updReparto(o.id, "entregado")} style={{ ...sB, flex:1, background:o.estadoReparto === "entregado" ? "#43a047" : "#e8edf5", color:o.estadoReparto === "entregado" ? "#fff" : "#555" }}>✅ Entregado</button>
            </div>
          </div>
        ))}
      </div>
      {modalOrder && (
        <Modal onClose={() => setModalOrder(null)}>
          ...
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:16 }}>
            {DIAS.map(d => (
              <button key={d} onClick={() => setFechaSel(d)} style={{ padding:"10px 14px", borderRadius:10, border:"none", cursor:"pointer", background:fechaSel === d ? "#1565c0" : "#e8edf5", color:fechaSel === d ? "#fff" : "#555", fontWeight:fechaSel === d ? 700 : 400, fontSize:13 }}>{d}</button>
            ))}
          </div>
...
function AdminApp({ user, onLogout }) {
  ...
  const handleImport = async () => {
    ...
    if (lines.length < 2) return alert("Pegá al menos una fila de datos + encabezado");
    ...
    const incoming = lines.slice(1).map(l => {
      const cols = l.split(sep);
      return { codigo: idxCod >= 0 ? cols[idxCod]?.trim() || "" : "", nombre: cols[idxNom]?.trim() || "", precio: parseFloat(cols[idxPre]?.replace(/[^0-9.]/g, "")) || 0, proveedor: idxProv >= 0 ? cols[idxProv]?.trim() || "" : "SIN PROVEEDOR" };
    }).filter(p => p.nombre && p.precio > 0);
    const base = [...(products.length > 0 ? products : INIT_PRODUCTS)];
    let actualizados = 0, nuevos = 0;
    incoming.forEach(item => {
      const idx = base.findIndex(p => String(p.codigo).trim() === String(item.codigo).trim() && item.codigo !== "");
      if (idx >= 0) { base[idx] = { ...base[idx], nombre: item.nombre, precio: item.precio, proveedor: item.proveedor }; actualizados++; }
      else { base.push({ ...item, id: Date.now() + Math.random() }); nuevos++; }
    });
    if (!confirm(`Se van a actualizar ${actualizados} artículos y agregar ${nuevos} nuevos. ¿Continuar?`)) return;
    setSaving(true);
    await apiPost("importArticulos", base);
    setProducts(base); setImportText(""); setShowImport(false);
  ...
  const handleImportStock = async () => {
    ...
    lines.slice(1).forEach(l => {
      ...
      const cant = parseInt(cols[idxCant]?.trim()) || 0;
      if (cod) { newStock[cod] = cant; actualizados++; }
    });
    if (!confirm(`¿Actualizar stock de ${actualizados} artículos?`)) return;
    setSaving(true);
    await apiPost("saveStock", newStock);
    setStock(newStock); setStockImportText(""); setShowStockImport(false);
  ...
  return (
    <div style={{ maxWidth:700, margin:"0 auto", fontFamily:"sans-serif", background:"#f4f6fb", minHeight:"100vh" }}>
      ...
      <Tabs tabs={["pedidos","articulos","stock"]} labels={[`📋 Pedidos${pendientes > 0 ? ` (${pendientes})` : ""}`, "📦 Artículos", "📊 Stock"]} active={tab} onChange={setTab} color={color} />
      <div style={{ padding:16 }}>
        {tab === "pedidos" && (
          <div>
            <div style={{ display:"flex", gap:6, marginBottom:12, flexWrap:"wrap" }}>
              {["todos","pendiente","transcripto","facturado"].map(e => (
                <button key={e} onClick={() => setFilterEstado(e)} style={{ padding:"6px 12px", borderRadius:20, border:"none", cursor:"pointer", background:filterEstado === e ? color : "#e8edf5", color:filterEstado === e ? "#fff" : "#555", fontSize:12, fontWeight:filterEstado === e ? 700 : 400 }}>
                  {e.charAt(0).toUpperCase() + e.slice(1)}{e === "pendiente" && pendientes > 0 ? ` (${pendientes})` : ""}
                </button>
              ))}
            </div>
            {filtOrders.length === 0 && <p style={{ color:"#888", textAlign:"center" }}>No hay pedidos</p>}
            {filtOrders.map(o => (
              <div key={o.id} style={{ ...cd, borderLeft:`4px solid ${o.urgente ? "#e53935" : "#43a047"}` }}>
                <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:4 }}>
                  <div>
                    <b style={{ fontSize:15 }}>{o.cliente}</b>
                    <span style={{ fontSize:11, marginLeft:8, background:o.tipo === "pedido" ? "#e3f0ff" : "#fff3e0", color:o.tipo === "pedido" ? "#1e3a5f" : "#e65100", borderRadius:6, padding:"2px 6px" }}>{o.tipo}</span>
                    <div style={{ fontSize:12, color:"#888" }}>{o.vendedor} · {o.fecha}</div>
                  </div>
                  <EstadoBadge estado={o.estado} estadoReparto={o.estadoReparto} />
                </div>
                ...
                <div style={{ marginTop:6 }}>
                  {o.items && o.items.map((i, idx) => <div key={idx} style={{ fontSize:12, color:"#444", padding:"1px 0" }}>• {i.nombre} x{i.qty} — ${(i.precio * i.qty).toLocaleString("es-AR")}</div>)}
                </div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:10, flexWrap:"wrap", gap:8 }}>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    ...
                    <button onClick={() => updEstado(o.id, "transcripto")} style={{ ...sB, background:o.estado === "transcripto" ? "#fb8c00" : "#e8edf5", color:o.estado === "transcripto" ? "#fff" : "#555" }}>📝 Transcripto</button>
        ...
        {tab === "articulos" && (
          <div>
            <div style={{ display:"flex", gap:8, marginBottom:12 }}>
              <button onClick={() => { setShowForm(true); setEditItem(null); }} style={{ ...bP(color), flex:2, marginTop:0 }}>➕ Agregar artículo</button>
              <button onClick={() => setShowImport(!showImport)} style={{ ...bP("#1565c0"), flex:1, marginTop:0 }}>📥 Importar lista</button>
            </div>
            {showImport && (
              <div style={{ ...cd, border:`2px solid #1565c0`, marginBottom:12 }}>
                <b style={{ color:"#1565c0" }}>📥 Importar lista de precios</b>
                <p style={{ fontSize:12, color:"#555", marginTop:6 }}>Copiá desde Excel: <b>Código, Nombre, Precio, Proveedor</b> (con encabezado). Actualiza por código, no borra nada.</p>
                <textarea value={importText} onChange={e => setImportText(e.target.value)}
                  placeholder={"Codigo\tNombre\tPrecio\tProveedor\n1234\tProducto X\t1500\tBECHLAB"}
                  style={{ width:"100%", height:100, borderRadius:8, border:"1px solid #ddd", padding:10, fontSize:12, boxSizing:"border-box", fontFamily:"monospace" }} />
            ...
            {(showForm || editItem) && (
              <div style={{ ...cd, border:`2px solid ${color}`, marginBottom:12 }}>
                <b style={{ color }}>{editItem ? "✏️ Editar" : "➕ Nuevo artículo"}</b>
            ...
            {filtProd.slice(0, 100).map(p => (
              <div key={p.id} style={{ ...cd, display:"flex", justifyContent:"space-between", alignItems:"center", padding:"8px 12px" }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <b style={{ fontSize:12 }}>{p.nombre}</b>
                  <div style={{ fontSize:10, color:"#888" }}>{p.codigo} · {p.proveedor}</div>
                </div>
                <b style={{ color, fontSize:13, margin:"0 10px" }}>${parseFloat(p.precio).toLocaleString("es-AR")}</b>
                <div style={{ display:"flex", gap:6 }}>
                  <button onClick={() => { setEditItem({ ...p }); setShowForm(false); }} style={{ padding:"4px 10px", borderRadius:8, border:"none", background:"#e3f0ff", color:"#1e3a5f", cursor:"pointer", fontSize:12 }}>✏️</button>
                  <button onClick={() => deleteArticulo(p.id)} style={{ padding:"4px 10px", borderRadius:8, border:"none", background:"#fdecea", color:"#e53935", cursor:"pointer", fontSize:12 }}>🗑️</button>
                </div>
              </div>
            ))}
          </div>
          ...
          <div>
            ...
            {showStockImport && (
              <div style={{ ...cd, border:"2px solid #1565c0", marginBottom:12 }}>
                ...
                <p style={{ fontSize:12, color:"#555", margin:"6px 0" }}>Copiá desde Excel con columnas <b>Codigo</b> y <b>Cantidad</b>. Solo actualiza los que pegás.</p>
                <textarea value={stockImportText} onChange={e => setStockImportText(e.target.value)}
...
function Header({ user, syncing, lastSync, onLogout }) {
  return (
    <div style={{ background:user.color, color:"#fff", padding:"14px 18px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
      <div>
        <b>{user.emoji} {user.name}</b> <span style={{ fontSize:12, opacity:.7 }}>{user.role}</span>
...
function Tabs({ tabs, labels, active, onChange, color }) {
  return (
    <div style={{ display:"flex", background:"#fff", borderBottom:"2px solid #e8edf5", overflowX:"auto" }}>
      {tabs.map((t, i) => (
        <button key={t} onClick={() => onChange(t)} style={{ flex:1, padding:"12px 8px", border:"none", background:"transparent", borderBottom:active === t ? `3px solid ${color}` : "3px solid transparent", color:active === t ? color : "#888", fontWeight:active === t ? 700 : 400, cursor:"pointer", fontSize:12, whiteSpace:"nowrap" }}>
          {labels[i]}
        </button>
      ))}
    </div>
...
function EstadoBadge({ estado, estadoReparto }) {
  ...
  if (estado === "transcripto") return <span style={{ fontSize:11, fontWeight:700, color:"#fb8c00" }}>📝 Transcripto</span>;
  return <span style={{ fontSize:11, fontWeight:700, color:"#1e3a5f" }}>⏳ Pendiente</span>;
}

function RepartoBadge({ estadoReparto, fechaReparto }) {
  if (estadoReparto === "entregado") return <span style={{ fontSize:12, fontWeight:700, color:"#43a047", background:"#e8f5e9", padding:"3px 8px", borderRadius:8 }}>✅ Entregado</span>;
...
const sB = { padding:"8px 10px", borderRadius:8, border:"none", cursor:"pointer", fontSize:12, fontWeight:600 };
